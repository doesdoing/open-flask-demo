...
def api_message(what):
    ...
    if len(new_dict) < 5:
                ...
                if what == 'user':
                    if data_info['level'] == 'admin':
                        data = sqlite3_plus.sqlite3_plus(
                            Path='./sql/'+what+'.db', Tab=what).find()
        ...
        if start and end:
            ...
            _ARR = {'ip': _IP, 'location': _LOCATION, 'projects': _PROJECTS,
                    'name': _NAME, 'src_name': _SYS_FILE, 'username': _USERNAME, 'level': _LEVEL}
            e1 = ''
            if _IP or _LOCATION or _PROJECTS or _NAME or _SYS_FILE or _USERNAME or _LEVEL:
                print(_ARR)
                for x in _ARR:
                    if _ARR[x]:
                        e1 = _find.find(target=x, start=start,
                                        end=end, data=data, value=_ARR[x])
...
def set_cookie():
    ...
    if request.method == 'POST':
        if name != '' and Pass != '':
            ...
            if value != []:
                if (name == value[0]['username'] and Pass == value[0]['password']):
...
def upload():
    if request.method == 'POST':
        file_class = ['txt', 'png', 'jpg', 'xls', 'JPG', 'PNG',
                      'xlsx', 'gif', 'GIF', 'doc', 'docx', 'ppt', 'pptx']
        f = request.files.getlist('file1')
        T_F=''
        Path = './static/uploads'
        for x in f:
            print(x)
            src_name = x.filename.rsplit('.', 1)[0]
            ext = x.filename.rsplit('.', 1)[1]
            if ext in file_class:
                unix_time = int(time.time())
                new_filename = random_plus().random_plus(EN='T')+str(unix_time)
                x.save(os.path.join(Path, new_filename+'.'+ext))
